...
echo -e "${YELLOW}--- VM 镜像 (适合虚拟机 / .img) ---${NC}"
if [ ${#vm_files[@]} -eq 0 ]; then echo "  [无]"; fi
...
echo ""
echo -e "${YELLOW}--- LXC 镜像 (适合容器 / .tar.*) ---${NC}"
if [ ${#lxc_files[@]} -eq 0 ]; then echo "  [无]"; fi
...
if [ "$mode" == "1" ]; then
    ...
    echo "[提示] 请确保您已在 Web 界面创建好 ID 对应的虚拟机壳子。"
    read -p "请输入目标 VM ID: " vmid
    
    if ! qm status $vmid >/dev/null 2>&1; then
        echo -e "${RED}[错误] VM $vmid 不存在。${NC}"; exit 1
    fi
    ...
    read -p "请选择存储编号 (默认 1): " s_idx; s_idx=${s_idx:-1}
    ...
    echo -e "${GREEN}完成！磁盘已注入 VM $vmid 并在 SCSI 0 挂载。${NC}"
...
# ==================== 模式 2: LXC 自动创建逻辑 ====================
    ...
    for i in "${!storage_list[@]}"; do echo " [$((i+1))] ${storage_list[$i]}"; done
    read -p "请选择存储 (默认 1): " s_idx; s_idx=${s_idx:-1}
    selected_storage="${storage_list[$((s_idx-1))]}"
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}完成！LXC $ctid 已创建成功。${NC}"
    else
        echo -e "${RED}失败！请检查镜像是否为 rootfs 格式。${NC}"
    fi
fi